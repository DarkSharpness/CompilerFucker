...
struct ASMbuilder final : IR::IRvisitorbase {
    ...
    std::vector <IR::node *> load_store_buffer;
    ...
    /* Return the binded virtual register. */
    virtual_register *get_virtual(IR::non_literal *);
    /* Load a constant to a random register. */
    virtual_register *get_virtual(IR::literal *);
    /* Try to get a virtual value. */
    virtual_register *get_virtual(IR::definition *);
