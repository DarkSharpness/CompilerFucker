...
bool local_optimizer::update_bin(IR::binary_stmt *__stmt) {
    ...
    auto &&__merge_symmetric = [__stmt]
        (IR::binary_stmt *__temp,IR::definition *__def,uint32_t __state) -> bool {
        /* (X - Y) + Y  --> X + 0 = X */
        if (__stmt->op == IR::binary_stmt::ADD) {
    ...
    auto &&__merge_lhs = [&,__stmt]() -> bool {
        ...
        auto *__def  = dynamic_cast <IR::non_literal *> (__rhs->new_def);
        if (!__temp ) return false;
    ...
    auto &&__merge_rhs = [&,__stmt]() -> bool {
        ...
        auto *__def  = dynamic_cast <IR::non_literal *> (__lhs->new_def);
    ...
    */
    ...
    if (replace_common(__stmt)) return true;
